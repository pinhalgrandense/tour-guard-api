...
import UserHelper, { Roles } from "App/Helpers/UserHelper";
...
export default class ManagersController {
  ...
  public async store({ request, response }: HttpContextContract) {
    const { full_name, email, password } = request.only([
      "full_name",
      "email",
      "password",
    ]);
    ...
    // adiciona o user como manager na tabela role_users

    const trx = await Database.transaction();
    try {
      await UserHelper.assignRole(user, Roles.MANAGER, trx);
    } catch (error) {
      await trx.rollback();
      return response
        .status(400)
        .json({ message: "Erro ao criar manager", error });
    }
    ...
    return response
      .status(201)
      .json({ message: "Manager criado com sucesso", manager });
  }
  ...
  public async update({ params, request, response }: HttpContextContract) {
    try {
      ...
      return response
        .status(400)
        .json({ message: "Erro ao atualizar manager", error });
    }
  }
  ...
  public async destroy({ params, response }: HttpContextContract) {
    try {
      ...
      return response
        .status(400)
        .json({ message: "Erro ao deletar manager", error });
    }
  }
}
